...
with film_category as (
    ...
    from {{ source('raw_sakila', 'film_category') }}
),
...
category as (
    ...
    from {{ source('raw_sakila', 'category') }}
),
...
language as (
    select
        ...
        last_update
    from {{ source('raw_sakila', 'language') }}
),
...
film as (
    ...
    from {{ source('raw_sakila', 'film') }}
)

select
    {{ dbt_utils.generate_surrogate_key(['f.film_id','f.film_last_update']) }} as film_skey, 
